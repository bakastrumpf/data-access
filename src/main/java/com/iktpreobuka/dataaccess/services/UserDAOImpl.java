...
public class UserDAOImpl implements UserDAO {
    ...
    public String uploadFile(MultipartFile file) throws IllegalStateException, IOException {
        if (file.equals(null)) {
            throw new MultipartException("File was not uploaded");
        }
    ...
    private boolean isEmailExist(UserEntity user) {
        ...
        if (userDb == null) {
            ...
            logger.warn("The user couldn't be saved, because email already exist");
            return true;
        }
    }
        ...
        try (FileInputStream inputStream = new FileInputStream(uploadedFile);
            ...
            while (input.hasNext()) {
                ...
                if (userObject.length > 0) {
                    ...
                    message += "There's something wrong with the file, so the user couldn't be saved.\r";
                    logger.warn("There's something wrong with the file, so the user couldn't be saved.");
                }
